...
if [ "$#" -ne 8 ]; then
    echo "Usage:"
    echo "  $0 PROD_NAMESPACE TOOLS_NAMESPACE APP_NAME EXTERNAL_DB_HOST EXTERNAL_DB EXTERNAL_DB_USER EXTERNAL_DB_PWD ENV[DEV,QA,SIT,UAT,PRE-PROD,PROD]]"
    exit 1
fi
...
# Add NEXUS URL 
sed -ie "s@URL@${NEXUS_ROUTE_URL}/repository/maven-all-public/@g" ./Infrastructure/templates73/settings.xml
...
oc create configmap settings.xml --from-file ./Infrastructure/templates73/settings.xml

# Reset back to URL in case need to change for PROD
sed -ie "s@${NEXUS_ROUTE_URL}/repository/maven-all-public/@URL@g" ./Infrastructure/templates73/settings.xml
