...
Note: 'tools' is currently hard-coded and as such, this has to be the Project canonical name.

1. create nexus + rh proxy repos

		cd ./ocp_pam_app_dev
		oc login -u <CLUSTER PUBLIC URL> -u <USERNAME> -p <PASSWORD>
		oc project tools
		# oc create -f nexus.yaml -n tools
		#./Infrastructure/scripts/setup_nexus.sh tools <CLUSTER PUBLIC URL>
